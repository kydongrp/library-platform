...
export async function GET(request: Request) {
  ...
  // Validated against the managed list, not a compile-time constant: a
  // category staff added must not come back as "unknown".
  if (category) {
    const allowed = await listCategories();
    if (!allowed.includes(category))
      return apiError(400, "bad_category", `Unknown category. One of: ${allowed.join(", ")}.`);
  }
